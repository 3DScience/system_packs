...
inline float CloudNoise(sampler2D tex, float2 pos, float c, float2 vel)
{
	return CalculateNoiseXZ(tex, float3(pos.x, 0.0, pos.y), _FogNoiseScale, 0.0, vel, _FogNoiseMultiplier, c);
}

inline float CloudFBM(float2 p, float c)
{
	float f1 = (CloudNoise(_FogNoise, p, c, _FogNoiseVelocity) * (1.0 - _FogWhispiness));
	float f2 = (CloudNoise(_FogNoise, p, c, _FogNoiseVelocity * (1.0 - _FogWhispinessChangeFactor)) * _FogWhispiness);
...
fixed ComputeCloudFBMFromXZ(float2 xz)
{
	...
	fixed f = CloudFBM(xz, c);
	f = saturate(1.0 - (pow(_FogSharpness, f - (1.0 - c))));

#if defined(ENABLE_CLOUDS_MASK)

	float2 maskRotated = RotateUV(xz, _FogNoiseMaskRotationSin, _FogNoiseMaskRotationCos);
	f *= CalculateNoiseXZ(_FogNoiseMask, float3(maskRotated.x, 0.0, maskRotated.y), _FogNoiseMaskScale, _FogNoiseMaskOffset, _FogNoiseMaskVelocity, 1.0, 0.0);
...
fixed4 ComputeCloudColor(float3 ray, fixed4 sunColor, out float3 worldPos)
{
	...
	return fixed4((_FogColor * fixed4(litCloud, 1.0) * densityColorMultiplier) + (_FogEmissionColor.rgb * _FogEmissionColor.a), alpha);
}
