...
Properties
    ...
    SubShader
	{
		...
		inline float lerpColor(float4 c)
		{
			...
			float t = _Time.y;
			float lerpMultiplier = saturate(ceil(t - c.g) + 0.000001); // if (t < c.g)
			float lerp1Scalar = saturate(((t - c.r) / max(0.000001, (c.g - c.r))));
			float lerp2Scalar = saturate(max(0, ((t - c.b) / max(0.000001, (c.a - c.b)))));
