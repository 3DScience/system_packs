...
inline float CalculateFogNoise3D(float3 pos, float3 rayDir, float rayLength, float scale, float3 velocity)
{
	...
	{
		n += tex3Dlod(_WeatherMakerNoiseTexture3D, float4(pos + (_WeatherMakerTime.x * velocity), -999.0)).a;
		pos += step;
	}
...
{
	return tex3Dlod(_WeatherMakerNoiseTexture3D, float4((pos * scale) + (_WeatherMakerTime.x * velocity), -999.0)).a;
}
